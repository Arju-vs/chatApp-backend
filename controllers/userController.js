...
export const signUp = async (req, res)=>{
    ...
    try {
        ...
        if(existingUser){
            res.json({success:"failed",message:"Already exists!!"})
        }else{
...
export const updateProfile = async (req, res)=>{
    try {
        ...
        if(!profilePic){
            updatedUser = await users.findByIdAndUpdate(userId, {bio, fullName} , {new:true})
        }else{
            const upload = await cloudinary.uploader.upload(profilePic)
            updatedUser = await users.findByIdAndUpdate(userId, { profilePic : upload.secure_url, bio , fullName}, {new:true})
        }
